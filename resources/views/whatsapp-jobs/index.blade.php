                    ...
                    <div class="card-body p-4">
                        <div class="row align-items-center">
                            <div class="col-md-3 border-end">
                                <label class="text-muted x-small fw-bold text-uppercase d-block mb-2">Eficiência
                                    Interna</label>
                            ...
                            <div class="col-md-9 ps-md-4">
                                ...
                                <div class="progress" style="height: 12px;">
                                    <div class="progress-bar bg-success"
                                        style="width: {{ $calcPercent($processados, $totalGeral) }}%"></div>
                                    <div class="progress-bar bg-danger"
                                        style="width: {{ $calcPercent($erros, $totalGeral) }}%"></div>
                                    <div class="progress-bar bg-warning"
                                        style="width: {{ $calcPercent($pendentes, $totalGeral) }}%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <tbody>
                        @forelse ($jobs as $job)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge {{ $statusMap[$job->status]['class'] ?? 'bg-secondary' }} rounded-pill px-3">
                                ...
                                <td class="small fw-bold">
                                    @if($job->evolution_status)
                                        <span class="text-primary"><i
                                                class="bi bi-whatsapp me-1"></i>{{ $job->evolution_status }}</span>
                                    @else
                                ...
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-xs btn-outline-secondary js-copy"
                                            data-value="{{ json_encode($job->campaignItem->generate($job->contact_id)) }}"
                                            title="Ver JSON Dinâmico">
                                ...
                                @if(auth()->user()->is_admin)
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-dark js-curl"
                                            data-endpoint="{{ $job->endpoint }}" data-payload="{{ json_encode($dynamicPayload) }}">
